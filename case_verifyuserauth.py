...
@ddt
class CaseVerifyUserAuth(unittest.TestCase):
    """
     发送验证码用例
    """
    do_log = HandleLogging().get_logger()
    do_excel = HandleExcel(TEST_DATAS_FILE_PATH2, sheetname='verifyUserAuth')
    ...
    @data(*case_list)
    def test_verify(self, one_case):
        self.do_log.info("\nRunning Testing Methon:{}".format(inspect.stack()[0][3]))
        new_data = Context.verify_parameterization(one_case.data)
